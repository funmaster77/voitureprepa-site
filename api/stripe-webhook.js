  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        // 3) Boost options au depot (Pack Photos+, Urgence, Remontada) — paiement OK :
        //    on marque ad.payment.status = "accepté" + paid=true. L'admin voit alors
        //    l'annonce avec ses options bien reglees lors de la moderation.
        if (meta.kind === "boost" && meta.ad_id) {
          const paymentJson = {
            required: true,
            amount: amountEuros,
            status: "accepté",
          ...
          const { error: boostErr } = await supa.from("ads").update({
            payment: paymentJson,
            paid: true,
            paid_at: new Date().toISOString()
          }).eq("id", meta.ad_id);
          if (boostErr) console.warn("[stripe-webhook] boost ad payment update error", boostErr);
          else console.log("[stripe-webhook] ad boost payment marked accepté", meta.ad_id);
        }

        // 4) Pack Pro (Premium/Performance) — activation/renouvellement du pack
