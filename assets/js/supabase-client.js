...
// Export pour app.js
// v15 : auto-hydratation de la session locale après login OAuth (Google/Facebook/Apple)
// Quand Supabase nous retourne SIGNED_IN avec une session valide, on remplit
// localStorage voitureprepa_session pour que le reste du code legacy le voie.
try {
  sb.auth.onAuthStateChange(async (event, session) => {
    ...
    try {
      ...
      let existing = null;
      try { existing = JSON.parse(localStorage.getItem(SESSION_KEY) || "null"); } catch(e) {}
      // Si déjà connecté avec le bon email, rien à faire
      if (existing && existing.email && existing.email.toLowerCase() === (session.user.email||"").toLowerCase()) return;
      // Récupère le profil Supabase (créé automatiquement par trigger pour OAuth)
      const profile = await sbGetMyProfile();
      const meta = session.user.user_metadata || {};
      // Pour les nouveaux users OAuth : meta.full_name ou meta.name est dispo
      const fullName = meta.full_name || meta.name || "";
      const guessedPrenom = (fullName.split(" ")[0] || "Membre");
      const guessedNom = (fullName.split(" ").slice(1).join(" ") || "");
      const sessionData = {
        id: (profile && profile.id) || session.user.id,
        type: (profile && profile.type) || "particulier",
        pack: (profile && profile.pack) || "gratuit",
        prenom: (profile && profile.prenom) || guessedPrenom,
        nom: (profile && profile.nom) || guessedNom,
        raison: (profile && profile.raison_sociale) || "",
        telephone: (profile && profile.telephone) || "",
        email: (profile && profile.email) || session.user.email || "",
        pack_expires_at: profile ? profile.pack_expires_at : null,
        garage_id: profile ? profile.garage_id : null,
        garage_label: profile ? profile.garage_label : null,
        siret: profile ? profile.siret : null
      };
      try { localStorage.setItem(SESSION_KEY, JSON.stringify(sessionData)); } catch(e) {}
    } catch (e) { console.warn("onAuthStateChange hydrate:", e); }
  });
} catch (e) { console.warn("onAuthStateChange setup:", e); }
