...
async function _pushThreadToSupabase(thread, data) {
  ...
  let otherId = null;
  // Cas 1 : on est l'acheteur, l'autre = vendeur = ad.owner_id
  const ad = (typeof loadUserAds === "function" ? loadUserAds() : []).find(a => String(a.id) === String(adIdNum));
  if (ad && ad.owner_id) {
    otherId = (ad.owner_id === user.id) ? null : ad.owner_id;
  }
  // Cas 2 : on est le vendeur, l'autre = acheteur → lookup profile par email
  ...
  if (!otherId) {
    console.warn("[pushThread] otherUserId non résolu pour", thread);
    return;
  }
  try {
