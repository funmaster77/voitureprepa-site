...
function loadUserAds() {
  ...
  //   par leur propriétaire. Le bypass admin (voir les annonces supprimées) est limité à :
  //     - la page /admin (détectée par URL),
  //     - OU une session locale dont le rôle est explicitement "admin".
  //   On NE bypass PLUS via le flag localStorage `vp_admin_session_ok` car il persiste
  //   après le logout admin et faisait apparaître les annonces supprimées aux utilisateurs
  //   normaux qui s'étaient connectés en admin auparavant.
  const isAdminPage = (typeof location !== "undefined") &&
                      /\/admin(\.html)?(\?|#|\/|$)/.test(location.pathname);
  let isAdminUser = false;
  try {
    const _s = getSession();
    if (_s && _s.role === "admin") isAdminUser = true;
  } catch (e) {}
  if (isAdminPage || isAdminUser) return _adsCache.slice();
