...
function openPackModal(packKey, typeFilter) {
  ...
  if (isInspection) {
    ...
    try {
      ...
      if (txtEl) txtEl.innerHTML = `Pour la formule <strong>${niLabel}</strong>, prévoyez environ <strong>${immobTxt || "—"}</strong> d'immobilisation du véhicule au garage. Le rendez-vous est fixé avec l'administrateur après votre souscription.`;
    } catch(e) {}
