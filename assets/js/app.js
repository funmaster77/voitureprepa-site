...
async function detectAfterBoostPack() {
  ...
  setTimeout(function () {
    ...
    if (afterClean === "ok") {
      ...
      try {
        (state.ad_ids || []).forEach(function(id) {
          ...
          if (!a) {
            // Cache pas encore pret : on patch directement Supabase avec just les nouvelles options
            if (typeof updateUserAd === "function") {
              var newOpts = [state.packLabel];
              var newBadges = state.badge ? ["pro", state.badge] : ["pro"];
              updateUserAd(id, { options: newOpts, badges: newBadges });
            }
            return;
          }
    ...
    try { history.replaceState(null, "", location.pathname); } catch(e) {}
    if (afterClean === "ok") {
      setTimeout(function () { location.href = "profil.html"; }, 200);
    }
  }, 200);
}
