...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // v20 — Si success_path/cancel_path contient deja un ?, on utilise & comme separateur
      success_url: baseUrl + (success_path || "/confirmation.html") +
                   (((success_path || "").indexOf("?") >= 0) ? "&" : "?") +
                   "stripe_session=" + "{CHECKOUT_SESSION_ID}",
      cancel_url:  baseUrl + (cancel_path  || "/tarifs.html") +
                   (((cancel_path  || "").indexOf("?") >= 0) ? "&" : "?") +
                   "stripe_cancelled=1",
      locale: "fr"
    });
