...
function loadUserAds() {
  // v22 — Sur les pages publiques + profil vendeur, on filtre les annonces "soft-supprimées"
  //   par leur propriétaire. Sur l'espace admin OU si l'utilisateur connecté a role=admin,
  //   on garde tout pour que l'admin voie l'historique à l'emplacement où l'annonce se trouvait
  //   (publiée, refusée, vendue...) et pour qu'il puisse consulter une annonce supprimée via Aperçu.
  //   Détecte /admin ET /admin.html (Vercel sert admin.html pour /admin sans extension).
  ...
  try {
    ...
    // Fallback : si le 2FA admin a été validé (flag posé par adminVerifyOtp),
    // on considère l'utilisateur comme admin pour cette session (la session locale
    // ne stocke pas toujours le rôle — il vient du profil Supabase).
    if (!isAdminUser && typeof localStorage !== "undefined" &&
        localStorage.getItem("vp_admin_session_ok") === "1") {
      isAdminUser = true;
    }
  } catch (e) {}
