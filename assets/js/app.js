...
async function updateTransaction(id, patch) {
  ...
  // Si l'id est un string "txn-..." (legacy localStorage), on tombe en fallback.
  if (typeof window !== "undefined" && window.VP_SB && window.VP_SB.client) {
    const numId = parseInt(id, 10);
    if (!isNaN(numId) && String(numId) === String(id)) {
      try {
        const { error } = await window.VP_SB.client
          .from("transactions").update(patch).eq("id", numId);
        if (!error) {
          // Met a jour le cache local pour reflet immediat
          const cached = _txnsCache.find(t => String(t.id) === String(numId));
          if (cached) Object.assign(cached, patch);
          return true;
        }
        console.warn("[updateTransaction] supabase err", error);
      } catch (e) {
        console.warn("[updateTransaction] supabase exception", e);
      }
    }
  }
