...
function subscribeProPack(packKey) {
  ...
  // désormais effectué côté serveur par /api/stripe-webhook en lisant les metadata.
  // La fonction vpPay (assets/js/payment.js) délègue au provider actif et redirige
  // le navigateur vers la page de paiement hébergée. Le callback synchrone "onPaid"
  // n'a plus lieu d'être ici.
  if (typeof window.vpPay !== "function") {
    alert("⚠ Module de paiement non chargé. Vérifiez que payment.js + stripe-checkout.js sont inclus dans cette page.");
    return;
  }
  window.vpPay({
    amount: montant,
