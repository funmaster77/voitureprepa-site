...
async function saveUserAd(ad) {
  ...
  try {
    ...
    const { data, error } = await VP_SB.client.from("ads").insert(payload).select("*, owner:profiles!owner_id(email)").maybeSingle();
    if (error) {
...
function doHeaderSearch() {
  ...
  let url;
  if (cat === "garage") url = "garages.html";
  else if (cat === "voiture") url = "annonces.html?type=voiture";
  else if (cat === "piece") url = "annonces.html?type=piece";
  else url = "annonces.html";
  const params = [];
...
function updateHeaderPlaceholder(cat) {
  ...
  if (!inp) return;
  if (cat === "voiture") inp.placeholder = "Marque, modèle, stage, type de préparation...";
  else if (cat === "piece") inp.placeholder = "Turbo, échappement, suspension, jantes...";
  else if (cat === "garage") inp.placeholder = "Nom du garage, ville, prestation...";
  else inp.placeholder = "Rechercher une voiture, une pièce, un garage...";
  fitHeaderCat();
}
