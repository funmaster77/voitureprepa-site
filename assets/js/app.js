...
function subscribeProPack(packKey) {
  ...
  const s = getSession();
  if (!s || s.type !== "pro") {
    // v21 — Si pas connecte en pro : propose d'aller creer un compte professionnel
    const goReg = confirm(
      "Pour souscrire au " + (PRO_PACKS[packKey] && PRO_PACKS[packKey].label || packKey) + ", vous devez avoir un compte Professionnel.\n\n" +
      "Voulez-vous etre redirige vers la page d'inscription Professionnel ?"
    );
    if (goReg) {
      location.href = "inscription.html?tab=pro&pack=" + encodeURIComponent(packKey);
    }
    return;
  }
