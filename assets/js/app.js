...
function _favsMigrationFaite() {
  const k = favsKey();
  if (!k) return true;
  try { return localStorage.getItem(k + "_migres") === "1"; } catch (e) { return true; }
}
function _marquerFavsMigres() {
  const k = favsKey();
  if (!k) return;
  try { localStorage.setItem(k + "_migres", "1"); } catch (e) {}
}
  ...
  try {
    ...
    if (!_favsMigrationFaite()) {
      ...
      for (const id of manquants) {
        ...
        catch (e) { console.warn("reprise du favori", id, e); }
      }
      _marquerFavsMigres();
    }
