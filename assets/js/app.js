...
async function applyAdminLinksVisibility() {
  let isAdmin = false;
  // 1er paint optimiste : flag local
  if (isAdminAuthedSync()) isAdmin = true;
  // Re-vérifie en async côté Supabase (source de vérité)
  if (window.VP_SB) {
    try {
      // v15 : VP_SB.getUser() retourne directement l'objet user
      const user = await VP_SB.getUser();
      if (!user) { isAdmin = false; localStorage.removeItem(ADMIN_LOCAL_FLAG); }
      else {
        const profile = await VP_SB.getMyProfile();
    ...
    } catch(e) { /* en cas d'erreur, on garde l'état optimiste */ }
  }
