...
/* =====================================================================
   ...
   Sans cookie (localStorage + sessionStorage), sans donnée personnelle :
...
function _vpSessionId() {
  try {
    const now  = Date.now();
    const last = parseInt(sessionStorage.getItem(VP_SESSION_TS) || "0", 10);
    let id = sessionStorage.getItem(VP_SESSION_KEY);
    if (!id || !last || (now - last) > VP_SESSION_TTL) {
      id = "s" + now.toString(36) + Math.random().toString(36).slice(2, 10);
      sessionStorage.setItem(VP_SESSION_KEY, id);
    }
    sessionStorage.setItem(VP_SESSION_TS, String(now));
    return id;
