...
(function () {
  ...
  let boite = null, x0 = 0, y0 = 0;
  document.addEventListener("touchstart", function (e) {
    const t = e.target;
    const b = (t && t.closest) ? t.closest(".an-img") : null;
    if (!b || b.querySelectorAll(".an-slide").length < 2) { boite = null; return; }
    boite = b; x0 = e.touches[0].clientX; y0 = e.touches[0].clientY;
  }, { passive: true });
  document.addEventListener("touchend", function (e) {
    if (!boite) return;
    const t = e.changedTouches[0];
    const dx = t.clientX - x0, dy = t.clientY - y0;
    // Geste franchement horizontal : sinon c'est un défilement de page ou un clic
    if (Math.abs(dx) > 40 && Math.abs(dx) > Math.abs(dy) * 1.5) {
      anCardSlide(boite, dx < 0 ? 1 : -1);
      e.preventDefault();   // évite l'ouverture de l'annonce juste après le balayage
    }
    boite = null;
  }, { passive: false });
})();
